"""Current V2 filters unchanged; only emitted signals own the cooldown."""
...
OUT_JSON = ROOT / "tmp" / "current_v2_emitted_cooldown_v12.json"
OUT_CSV = ROOT / "tmp" / "current_v2_emitted_cooldown_v12_trades.csv"


def replay_strict(data: pd.DataFrame, row: dict[str, Any]) -> tuple[list[dict[str, Any]], dict[str, int]]:
    ...
    counts = {"wait": 0, "veto": 0, "bookRejected": 0, "incident": 0, "emitted": 0}
    for pos in range(warmup, len(data) - 610):
        ...
        trades.append({
            ...
            "branch": "current_v2_strict_emitted_cooldown", "priority": 0,
            "reason": decision.get("reason"), "confirmation_votes": votes,
...
def replay_sources() -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for source in SOURCES:
        ...
        trades, counts = replay_strict(data, row)
        kept = 0
        for trade in trades:
            ...
            if not start <= timestamp <= end:
                continue
            item = {
                "time": timestamp, "signal": trade["signal"], "branch": trade["branch"],
                "priority": 0, "reason": trade.get("reason"), "source": source.name,
                "confirmation_votes": trade.get("confirmation_votes"),
            }
            for delay in DELAYS:
...
def run() -> dict[str, Any]:
    ...
    report = {
        "method": {
            ...
            "filters": "current V2 accepted status unchanged plus uniform trailing-book confirmation",
            "onlyExecutionChange": "veto and book rejection do not own cooldown; emitted signal does",
            "supplement": "exhaustion/order-book branch under the same strategy and cooldown",
            "cooldownSec": 600, "amountU": 5,
        },
        "sourceAudit": audit,
        "inputSignals": {"strictCurrent": len(current), "supplement": len(supplement)},
        "periods": {
