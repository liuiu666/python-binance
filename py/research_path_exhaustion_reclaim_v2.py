"""Fixed high-efficiency exhaustion reclaim strategy.

The strategy fades a one-sided ten-minute auction only after the final minute
prints a meaningful move against that auction.  It never fades an accelerating
path and never uses the future outcome to classify the setup.
"""
...
OUT_JSON = ROOT / "tmp" / "path_exhaustion_reclaim_v2.json"
OUT_CSV = ROOT / "tmp" / "path_exhaustion_reclaim_v2_trades.csv"
DELAYS = (0, 5, 6, 10)
...
def build_candidates(minutes: pd.DataFrame) -> pd.DataFrame:
    ...
    frame = frame[setup & frame.move_bps.notna()].replace([np.inf, -np.inf], np.nan).dropna(
...
def run() -> dict[str, Any]:
    ...
    report = {
        "method": {
            ...
            "rule": "fade a high-efficiency extreme only after a >=2bp opposing final minute",
            "cooldownSec": 600,
        },
