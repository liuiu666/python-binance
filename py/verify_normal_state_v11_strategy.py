...
from second_backtest.normal_state_v9 import load_default_features


REFERENCE_CSV = ROOT / "tmp" / "normal_state_v9_state_gate_trades.csv"
OUT_JSON = ROOT / "tmp" / "normal_state_v11_strategy_verify.json"
REFERENCE_KEY = "D5_A5_V6_CONSENSUS_2OF5_UPPER_edge_persistence_lt6"
...
def _read_reference() -> pd.DataFrame:
    ...
    return ref.sort_values("idx").drop_duplicates(["idx", "settle_idx", "signal", "entry"], keep="last")
...
def run() -> dict:
    ...
    cfg = NormalStateV11Config()
    rows = generate_normal_state_v11_signals(bars, cfg, features=features)
    ref = _read_reference()
