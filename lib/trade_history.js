...
function auditOrderRow(row, ticks, payoutRate) {
...
function shadowAuditRows(auditRows, payoutRate) {
  const byId = new Map();
  ...
  for (const [id, row] of byId.entries()) {
    ...
    const duration = String(open.duration || row.duration || "");
    const effectivePayoutRate = payoutRateForDuration(duration, payoutRate);
    const status = settle.status || (settle.settlePrice != null ? settleStatus(open.direction, open.strikePrice, settle.settlePrice) : "pending");
    out.push({
      id: "shadow_audit|" + id,
      ...
      settleTime: Number(settle.settleTime || (Number(open.openTime || open.serverTime || 0) + (Number(duration) || 0) * 60000)),
      openPrice: open.strikePrice,
      closePrice: settle.settlePrice ?? null,
      status,
      payoutRate: effectivePayoutRate,
...
function buildLiveOrderHistory(options = {}) {
  ...
  for (const row of options.auditRows || []) {
    if (row.event === "order_done") {
      const item = auditOrderRow(row, ticks, payoutRate);
      if (item) rows.push(item);
    }
    if (row.event === "order_abort") {
      const item = auditAbortRow(row);
      if (item) rows.push(item);
    }
  }

  rows.push(...shadowAuditRows(options.auditRows || [], payoutRate));
