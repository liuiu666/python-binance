...
function shadowAuditRows(auditRows, payoutRate, options = {}) {
  ...
  for (const [id, row] of byId.entries()) {
    ...
    out.push({
      ...
      executionStrikePrice: open.executionStrikePrice ?? null,
      executionOpenTime: open.executionOpenTime ?? null,
      executionDelayMs: open.executionDelayMs ?? (
        open.executionOpenTime && (open.openTime || open.serverTime)
          ? Number(open.executionOpenTime) - Number(open.openTime || open.serverTime)
          : null
      ),
