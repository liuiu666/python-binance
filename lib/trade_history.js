...
function auditOrderRow(row, ticks, payoutRate, options = {}) {
  ...
  return {
    ...
    llm_decision_id: row.llm_decision_id ?? row.signal?.llm_decision_id ?? null,
    llm_model: row.llm_model ?? row.signal?.llm_model ?? options.llmLogsByDecisionId?.[row.llm_decision_id ?? row.signal?.llm_decision_id]?.llm_model ?? null,
    llm_prompt: row.llm_prompt ?? row.signal?.llm_prompt ?? options.llmLogsByDecisionId?.[row.llm_decision_id ?? row.signal?.llm_decision_id]?.llm_prompt ?? null,
    llm_response: row.llm_response ?? row.signal?.llm_response ?? options.llmLogsByDecisionId?.[row.llm_decision_id ?? row.signal?.llm_decision_id]?.llm_response ?? null,
...
function auditAbortRow(row) {
  ...
  return {
    ...
    modelLatencyMs: row.signal?.model_latency_ms ?? null,
    llm_decision_id: row.llm_decision_id ?? row.signal?.llm_decision_id ?? null,
    llm_model: row.llm_model ?? row.signal?.llm_model ?? null,
    llm_prompt: row.llm_prompt ?? row.signal?.llm_prompt ?? null,
    llm_response: row.llm_response ?? row.signal?.llm_response ?? null,
...
function shadowAuditRows(auditRows, payoutRate, options = {}) {
  ...
  for (const [id, row] of byId.entries()) {
    ...
    out.push({
      ...
      llm_decision_id: open.llm_decision_id ?? null,
      llm_model: open.llm_model ?? options.llmLogsByDecisionId?.[open.llm_decision_id]?.llm_model ?? null,
      llm_prompt: open.llm_prompt ?? options.llmLogsByDecisionId?.[open.llm_decision_id]?.llm_prompt ?? null,
      llm_response: open.llm_response ?? options.llmLogsByDecisionId?.[open.llm_decision_id]?.llm_response ?? null,
...
function buildLiveOrderHistory(options = {}) {
  ...
  for (const row of options.auditRows || []) {
    if (row.event === "order_done") {
      const item = auditOrderRow(row, ticks, payoutRate, { now, currentPrice: options.currentPrice, llmLogsByDecisionId: options.llmLogsByDecisionId });
  ...
  rows.push(...shadowAuditRows(options.auditRows || [], payoutRate, { ticks, now, currentPrice: options.currentPrice, llmLogsByDecisionId: options.llmLogsByDecisionId }));
