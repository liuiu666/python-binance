const BACKTEST_PRESETS = {
  ...
  SECOND: {
    20: { wr: 66.67, tradesPerDay: 8.5, trades: 27, maxLoss: 2, sampleHours: 76.23 },
    22: { wr: 71.43, tradesPerDay: 8.82, trades: 28, maxLoss: 2, sampleHours: 76.23 }
  },
  SECOND_CHIP: {
    ...
    WIDE_3600_FLOW: { wr: 76.67, tradesPerDay: 9.45, trades: 30, maxLoss: 2, sampleHours: 76.23 }
  }
};
...
const DEFAULT_STRATEGY_VARIANTS = [
  ...
  { id: "BTC_10min_SECOND_4200_22", base: "SECOND", label: "秒级正态 70m 22/78", amount: "10", tailPct: 0.22, enabled: true, tradeEnabled: true, lookbackSec: 4200, horizonSec: 600, gapSec: 1200, secondFilter: "none" },
...
function backtestForNormalizedVariant(variant) {
  ...
  if (variant.base === "SECOND") {
    ...
    if (
      lookback === 4200 &&
      Math.abs(tailPct - 0.22) < 1e-9 &&
      gap === 1200
    ) {
      return BACKTEST_PRESETS.SECOND[22];
    }
    if (
      lookback === 3600 &&
      Math.abs(tailPct - 0.2) < 1e-9 &&
      gap === 1800
    ) {
      return BACKTEST_PRESETS.SECOND[20];
    }
    return backtestForVariant(variant.base, variant.tailPct);
  }
  if (variant.base !== "SECOND_CHIP") return backtestForVariant(variant.base, variant.tailPct);
  const lookback = Number(variant.lookbackSec);
  ...
  if (
    ...
    Math.abs(target - 0.2) < 1e-9 &&
    binSize === 100 &&
    Math.abs(breakPct - 0.005) < 1e-9 &&
    gap === 600 &&
    chipFilter === "flow_reversal"
  ) {
    return BACKTEST_PRESETS.SECOND_CHIP.WIDE_3600_FLOW;
  }
  return null;
}
...
function normalizeVariants(input) {
  ...
  if (!out.some(v => v.id === "BTC_10min_SECOND_CHIP_3600_WIDE_FLOW")) {
