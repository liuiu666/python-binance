...
function variantLabel(variant, index) {
  ...
  if (variant.base === "llm_direction") return "GLM-5.2 方向预测";
...
function normalizeVariant(raw, index) {
  ...
  // LLM 是固定的影子策略槽位：身份不可伪造，且任何非严格 true 的开关都按关闭处理。
  if (base === "llm_direction") {
    const durationMin = sanitizePositiveInt(input.duration, 10, 1, 120);
    return {
      id: "BTC_10min_LLM_GLM52",
      base: "llm_direction",
      ...
      shadow: true,
      lookbackSec: sanitizePositiveInt(input.lookbackSec, 600, 60, 21600),
      horizonSec: durationMin * 60,
      gapSec: sanitizePositiveInt(input.gapSec, durationMin * 60, 0, 21600),
      backtest: null
    };
  }
  ...
  if (out.backtest && out.backtest.shadowObservationRequired === true) {
    out.tradeEnabled = false;
  }
...
function normalizeVariants(input) {
  ...
  const onlyCurrent = raw.filter(item => item && (
    ...
    item.base === "SECOND_BRANCH_VOTE_STARTUP_V1" ||
    item.base === "SECOND_MULTI_NORMAL_HF_STABLE_V1" ||
    item.base === "SECOND_MULTISCALE_PHASE_GATE_V1" ||
    item.base === "llm_direction"
  ));
  const defaultLlm = DEFAULT_STRATEGY_VARIANTS.find(item => item.base === "llm_direction");
  // LLM 是固定且默认关闭的策略槽位；升级旧配置时也必须补入，才能在页面上显式启用。
  const source = onlyCurrent.length
    ? (onlyCurrent.some(item => item.base === "llm_direction")
      ? onlyCurrent
      : [...onlyCurrent.slice(0, 7), defaultLlm])
