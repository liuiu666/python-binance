...
function normalizeVariant(raw, indexByBase) {
  ...
  const base = input.base === "SAFE" ? "SAFE" : "TAKER";
  const tailPct = sanitizeTailPct(input.tailPct, 0.2);
  const index = Number(indexByBase[base] || 0);
  indexByBase[base] = index + 1;
  const amount = Number(input.amount);
  return {
    id: String(input.id || variantId(base, tailPct, index)),
    base,
    label: variantLabel(base, tailPct),
    amount: Number.isFinite(amount) && amount > 0 ? String(input.amount) : (base === "SAFE" ? "5" : "10"),
    tailPct,
...
function migrateLegacyVariants(input) {
  ...
  return [
    { id: "BTC_10min_SAFE", base: "SAFE", amount: amounts.BTC_10min_SAFE || "5", tailPct: params.BTC_10min_SAFE?.tailPct ?? 0.2, enabled: true },
    { id: "BTC_10min_TAKER", base: "TAKER", amount: amounts.BTC_10min_TAKER || "10", tailPct: params.BTC_10min_TAKER?.tailPct ?? 0.2, enabled: true }
  ];
}
...
function normalizeVariants(input) {
  ...
  const indexByBase = { SAFE: 0, TAKER: 0 };
  const seen = new Set();
  const out = [];
  for (const item of withDefaults.slice(0, 12)) {
    const next = normalizeVariant(item, indexByBase);
    let id = next.id;
    if (seen.has(id)) id = `${variantId(next.base, next.tailPct, indexByBase[next.base])}_${out.length + 1}`;
    seen.add(id);
    out.push({ ...next, id });
  }
  if (!out.some(v => v.base === "SAFE")) out.unshift(normalizeVariant(DEFAULT_STRATEGY_VARIANTS[0], { SAFE: 0, TAKER: 0 }));
  if (!out.some(v => v.base === "TAKER")) out.push(normalizeVariant(DEFAULT_STRATEGY_VARIANTS[1], { SAFE: 1, TAKER: 0 }));
  return out;
}
...
function publicTradeConfig(config) {
  ...
  return {
    ...
    strategyParams: Object.fromEntries(cfg.strategyVariants.map(v => [v.id, { tailPct: v.tailPct }]))
  };
}
