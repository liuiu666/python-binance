...
# Raw signals based on 1m optimization:
df['raw_call'] = 0
df['raw_put'] = 0

# 1. Bearish Trend -> Call only (Z < -1.5 & RSI < 25)
idx_bear = df['regime'] == 'Bearish Trend'
df.loc[idx_bear & (df['zscore'] < -1.5) & (df['rsi'] < 25), 'raw_call'] = 1

# 2. Bullish Trend -> Put only (Z > 2.2 & RSI > 75)
idx_bull = df['regime'] == 'Bullish Trend'
df.loc[idx_bull & (df['zscore'] > 2.2) & (df['rsi'] > 75), 'raw_put'] = 1

# 3. Quiet Ranging -> Call only (Z < -2.2 & RSI < 30) - based on optimize output
idx_range = df['regime'] == 'Quiet Ranging'
df.loc[idx_range & (df['zscore'] < -2.2) & (df['rsi'] < 30), 'raw_call'] = 1
