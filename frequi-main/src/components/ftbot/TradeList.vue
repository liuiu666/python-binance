...
const tableFields = ref([
  ...
  {
    field: 'profit',
    header: props.activeTrades ? '当前盈亏 %' : '盈亏 %',
  },
  { field: 'open_timestamp', header: '开仓日期' },
  ...(props.activeTrades
    ? [{ field: 'actions', header: '' }]
    : [
        { field: 'close_timestamp', header: '平仓日期' },
        { field: 'exit_reason', header: '退出原因' },
      ]),
]);
...
async function removeTradeHandler(item: Trade) {
  if (
    await confirm({
      title: '删除交易',
      description: '此操作无法撤销。',
      message: `确定要删除交易 ${item.trade_id} (交易对 ${item.pair})？`,
      confirmText: '确认',
    })
  ) {
