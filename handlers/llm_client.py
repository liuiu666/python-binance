...
class LLMClient:
    ...
    def get_position_audit(self, position_data, market_data):
        ...
        **请重点分析此数据，判断是否有主力压盘或托单**
        ...
        [1H 大周期趋势参考]
        - 趋势: {market_data.get('h1_timeframe_trend', {}).get('trend')}
        - MA20: {market_data.get('h1_timeframe_trend', {}).get('ma20')}
        - 布林上轨: {market_data.get('h1_timeframe_trend', {}).get('boll_upper')}
        - 布林下轨: {market_data.get('h1_timeframe_trend', {}).get('boll_lower')}
        ...
        5. **检查挂单合理性 (动态调整)**：
           - **关键**: 如果当前没有止损单 (open_orders 中无 STOP 类订单)，**必须**建议设置止损 (SET_SL)。
           - **止损定价原则**: 严禁使用固定的 ATR 倍数 (如 "开仓价 +/- 1*ATR")。**必须**基于 **1H 级别的关键支撑/压力位** (如 MA20, 布林轨) 来设置止损。只有在找不到明显支撑压力时，才结合 ATR 设定一个宽幅止损。
           - **止盈目标**: 同样参考 **1H 级别的压力位 (做多)** 或 **支撑位 (做空)**。
           ...
           - **重要**: 盈利时尽量保护利润，但不要机械地要求必须高于/低于开仓价，应以技术面为准。
        
        请输出 JSON: 
        - action (HOLD / CLOSE / ADD / REDUCE)
        - reason (简短理由)
        - confidence (0-100)
        - suggested_stop_loss (数值类型，基于当前支撑/压力位计算的理想止损价，必须提供)
        - adjustment (可选对象):
    ...
    def _build_prompt(self, data):
        ...
        **这是最重要的判断依据，请优先参考**
        ...
        2. 1H (关键支撑压力):
           - 趋势: {data.get('h1_timeframe_trend', {}).get('trend')}
           - MA20: {data.get('h1_timeframe_trend', {}).get('ma20')}
           - 布林上轨: {data.get('h1_timeframe_trend', {}).get('boll_upper')}
           - 布林下轨: {data.get('h1_timeframe_trend', {}).get('boll_lower')}
        ...
        [硬性风控规则]
        1. 手续费+滑点成本约为 0.15% (双边)。如果预期利润空间 < 0.4%，请直接观望 (HOLD)，不要为了蝇头小利冒险。
        2. **量价配合 (OI)**: 
           - 价格上涨 + OI 下跌 = 空头平仓 (诱多)，上涨不可持续 -> 谨慎。
           - 价格下跌 + OI 下跌 = 多头止损，下跌动能减弱 -> 谨慎追空。
           - 只有 **价格与 OI 同向增长** (真突破) 才值得高信心介入。
        3. **大周期共振**: 如果 1m 信号与 1H 趋势 (MA20方向) 完全逆势，且无爆发性成交量 (3倍以上) 配合，严禁给出高信心信号 (Confidence < 60)。
        4. 如果资金费率极高 (>0.05%) 且趋势不明确，请避免做多。
        5. 如果 10m 成交量没有放大且价格动量不足，避免给出高信心信号。
        ...
        [止盈止损建议]
        - **止损过于敏感是当前的主要问题**。请务必参考 **1H (一小时线)** 的关键支撑/压力位 (如 MA20, 布林轨) 来设置止损，不要仅仅盯着 1m 线。
        - 止损应设置在 1H 结构的下方 (做多) 或 上方 (做空)，给波动留出足够的呼吸空间。
        - **止盈目标 (Take Profit)**: 同样参考 **1H 级别的压力位 (做多)** 或 **支撑位 (做空)**。
        - 盈亏比 (Reward/Risk Ratio) 至少 2.0，如果趋势强劲，可以看向更大的空间。
        """
